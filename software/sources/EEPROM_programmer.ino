// Parallel EEPROM Programmer for 28C256 and 28C64B
//
...
// shifting out address by using hardware SPI
...
// waits for write cycle to finish using data polling
...
// writes up to 64 bytes; bytes have to be page aligned (28C256 and 26C64B only)
...
// reads contents of EEPROM and sends it as binary data
void readBinary(uint16_t addr, uint16_t count) {
  ReadLEDon;
  while (count) {
    Serial.write(readDataByte(addr++));
    count--;
  }
  ReadLEDoff;
}
...
// writes binary data to a memory page
...
// converts byte nibble into hex character and prints it via serial
...
// converts byte into hex characters and prints it via serial
...
// converts word into hex characters and prints it via serial
